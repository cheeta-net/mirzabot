...
function activecron()
{
    ...
    $cronCommands = [
        ...
        "*/2 * * * * curl https://$domainhosts/cronbot/puzzlenetpay.php",
...
function getPuzzlenetpayApiBaseUrl()
{
    $url = select("PaySetting", "ValuePay", "NamePay", "puzzlenetpay_api_url", "select")['ValuePay'];
    $url = rtrim(trim($url), '/');
    return $url;
}
function createPayPuzzlenetpay($amount, $chat_id)
{
    $apiKey = select("PaySetting", "ValuePay", "NamePay", "puzzlenetpay_api_key", "select")['ValuePay'];
    $baseUrl = getPuzzlenetpayApiBaseUrl();
...
function verifyPayPuzzlenetpay($payment_id)
{
    $apiKey = select("PaySetting", "ValuePay", "NamePay", "puzzlenetpay_api_key", "select")['ValuePay'];
    $baseUrl = getPuzzlenetpayApiBaseUrl();
    if ($apiKey == "0" || empty($baseUrl)) {
    ...
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => $baseUrl . '/api/payment/' . urlencode($payment_id),
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 30,
