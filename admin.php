    ...
    if (count($statispay) != 0) {
        foreach ($statispay as $tracepay) {
            $status_var = [
                ...
                'PuzzleNetPay' => $textbotlang['textbot']['puzzleNetPay'],
...
} elseif ($datain == "puzzlenetpaysetting" && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $puzzlenetpaykeyboard, 'HTML');
} elseif ($text == $textbotlang['keyboard']['puzzleNetPayApiKey'] && $adminrulecheck['rule'] == "administrator") {
    $PaySetting = select("PaySetting", "ValuePay", "NamePay", "puzzlenetpay_api_key")['ValuePay'];
    $textpuzzlenetpay = sprintf($textbotlang['Admin']['adminphp']['ask_send_api'], $PaySetting);
    sendmessage($from_id, $textpuzzlenetpay, $backadmin, 'HTML');
    step('puzzlenetpay_api_key', $from_id);
} elseif ($user['step'] == "puzzlenetpay_api_key") {
    sendmessage($from_id, $textbotlang['Admin']['SettingnowPayment']['saveApi'], $puzzlenetpaykeyboard, 'HTML');
    update("PaySetting", "ValuePay", $text, "NamePay", "puzzlenetpay_api_key");
    step('home', $from_id);
} elseif ($text == $textbotlang['keyboard']['puzzleNetPayApiUrl'] && $adminrulecheck['rule'] == "administrator") {
    $PaySetting = select("PaySetting", "ValuePay", "NamePay", "puzzlenetpay_api_url")['ValuePay'];
    $textpuzzlenetpay = sprintf($textbotlang['Admin']['adminphp']['ask_enter_api'], $PaySetting);
    sendmessage($from_id, $textpuzzlenetpay, $backadmin, 'HTML');
    step('puzzlenetpay_api_url', $from_id);
} elseif ($user['step'] == "puzzlenetpay_api_url") {
    $apiUrl = rtrim(trim($text), '/');
    sendmessage($from_id, $textbotlang['Admin']['SettingnowPayment']['saveApi'], $puzzlenetpaykeyboard, 'HTML');
    update("PaySetting", "ValuePay", $apiUrl, "NamePay", "puzzlenetpay_api_url");
    step('home', $from_id);
...
if ($datain == "settimecornremove" && $adminrulecheck['rule'] == "administrator") {
    ...
    $puzzlenetpay = getPaySettingValue('statuspuzzlenetpay', 'offpuzzlenetpay');
    ...
    $puzzlenetpaystatus = [
        'onpuzzlenetpay' => $textbotlang['Admin']['Status']['statuson'],
        'offpuzzlenetpay' => $textbotlang['Admin']['Status']['statusoff']
    ][$puzzlenetpay];
    $zarinpalstatus = [
    ...
    $Bot_Status = json_encode([
        'inline_keyboard' => [
            ...
            [
                ['text' => $textbotlang['keyboard']['settings'], 'callback_data' => "puzzlenetpaysetting"],
                ['text' => $puzzlenetpaystatus, 'callback_data' => "editpayment-puzzlenetpay-$puzzlenetpay"],
                ['text' => $textbotlang['keyboard']['puzzleNetPayGateway'], 'callback_data' => "puzzlenetpay"],
            ],
            [
    ...
    } elseif ($type == "puzzlenetpay") {
        if ($value == "onpuzzlenetpay") {
            $valuenew = "offpuzzlenetpay";
        } else {
            $valuenew = "onpuzzlenetpay";
        }
        update("PaySetting", "ValuePay", $valuenew, "NamePay", "statuspuzzlenetpay");
...
} elseif ($text == $textbotlang['keyboard']['cashbackPuzzleNetPay']) {
    sendmessage($from_id, $textbotlang['Admin']['adminphp']['ask_send_user_payment_sub_1'], $backadmin, 'HTML');
    step("getcashpuzzlenetpay", $from_id);
} elseif ($user['step'] == "getcashpuzzlenetpay") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['Admin']['agent']['invalidValue'], $backadmin, 'HTML');
        return;
    }
    sendmessage($from_id, $textbotlang['users']['extraVolume']['changedPrice'], $puzzlenetpaykeyboard, 'HTML');
    step("home", $from_id);
    update("PaySetting", "ValuePay", $text, "NamePay", "chashbackpuzzlenetpay");
...
} elseif ($text == $textbotlang['keyboard']['minAmountPuzzleNetPay']) {
    sendmessage($from_id, $textbotlang['Admin']['adminphp']['ask_send_amount_3'], $backadmin, 'HTML');
    step("getmainpuzzlenetpay", $from_id);
} elseif ($user['step'] == "getmainpuzzlenetpay") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['Admin']['agent']['invalidValue'], $backadmin, 'HTML');
        return;
    }
    sendmessage($from_id, $textbotlang['Admin']['adminphp']['ok_amount_set_1'], $puzzlenetpaykeyboard, 'HTML');
    step("home", $from_id);
    update("PaySetting", "ValuePay", $text, "NamePay", "minbalancepuzzlenetpay");
} elseif ($text == $textbotlang['keyboard']['maxAmountPuzzleNetPay']) {
    sendmessage($from_id, $textbotlang['Admin']['adminphp']['ask_send_amount_4'], $backadmin, 'HTML');
    step("getmaaxpuzzlenetpay", $from_id);
} elseif ($user['step'] == "getmaaxpuzzlenetpay") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['Admin']['agent']['invalidValue'], $backadmin, 'HTML');
        return;
    }
    sendmessage($from_id, $textbotlang['Admin']['adminphp']['ok_amount_set_2'], $puzzlenetpaykeyboard, 'HTML');
    step("home", $from_id);
    update("PaySetting", "ValuePay", $text, "NamePay", "maxbalancepuzzlenetpay");
...
} elseif ($text == $textbotlang['keyboard']['setEducationPuzzleNetPay'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['adminphp']['ask_send_tutorial'], $backadmin, 'HTML');
    step("helppuzzlenetpay", $from_id);
} elseif ($user['step'] == "helppuzzlenetpay") {
    if ($text) {
        if (intval($text) == 2) {
            update("PaySetting", "ValuePay", "0", "NamePay", "helppuzzlenetpay");
        } else {
            $data = json_encode(array(
                'type' => "text",
                'text' => $text
            ));
            update("PaySetting", "ValuePay", $data, "NamePay", "helppuzzlenetpay");
        }
    } elseif ($photo) {
        $data = json_encode(array(
            'type' => "photo",
    ...
    step('home', $from_id);
    sendmessage($from_id, $textbotlang['Admin']['adminphp']['ok_success_tutorial'], $puzzlenetpaykeyboard, 'HTML');
